...
library(reshape2)
library(xlsx)
setwd("~/Rprojects_git_fix/Pangolin_GenomeAssign/")
...
ind<-read_delim("data/OriGen_input/Pangolin.111breeding_b.4Origen.loc.anonymize.txt",delim="\t") %>% rename(Sample_Name=Sample)
cv<-read.xlsx("data/pangolin_samples_for_code.xlsx",sheetIndex=1) %>%  dplyr::select(UCLA.sample.ID,Region_K5) %>% rename(Sample_Name=UCLA.sample.ID)
...
par(mfrow=c(1,1))
pdf('results/OriGen_results/Pangolin.nofilt.OriGen_clipped.ovlp.grey.km2.pdf')
...
km1 %>% left_join(rub)  %>% dplyr::select(Sample_Name,collection,repunit,inferred_repunit,Region_K5,Latitude,Longitude,everything()) %>% mutate(error_km=value/1000) %>% dplyr::select(-Var2,-value) %>% write.table("results/OriGen_results/Pangolin.Origen.self_assessment.rubias.InferGroup.obscLatLong.filtprob.Error2.txt",row.names=F,quote=F,sep="\t")
...
par(mfrow=c(1,2),mar=c(3,3,1,1),mgp=c(1.5,0,0),tck=0.01)
par(mfrow=c(1,1))
pdf('results/OriGen_results/Pangolin.filt_highprob.obscLatLong.OriGen_clipped.ovlp.grey.km2.pdf')
hist(km1$value/1000,main="",xlab="Distance Error (km)",breaks=20,col="gray40",border="white")
dev.off()


my_cols <- paste(c('#CC0000','#CC0000','#FFCC00','#33FFFF','#009933','#9933CC'),"80",sep="")
...
rectangled <- ggplot() +
  ...
  geom_sf(data = africa_geo, fill = NA, size = 0.15) +
  ...
  theme_bw() + theme(legend.key.size = unit(0.3, "cm"))+
...
ggsave("results/OriGen_results/Pangolin.OriGen_accuracy.nofilt.outline.clip.pdf")
